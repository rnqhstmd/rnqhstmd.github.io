...
자정이 지나면 날짜가 바뀌고 새로운 키가 생성된다. 새벽 00:00에는 오늘 데이터가 전혀 없다. 랭킹이 비어있는 상태다.

사용자가 자정 직후에 랭킹을 조회하면 아무것도 나오지 않는다.

### 잘못된 접근: 단순 복사

처음에는 자정에 전날 키를 오늘 키로 복사하는 방식을 생각했다.
...
문제: 오늘 활동이 쌓이면서 전날 데이터가 섞인다. 오늘 주문 0건인 상품이 어제 주문 많다는 이유로 상위에 남는다.

### 올바른 접근: Score Carry-Over (점수 이월)

핵심 아이디어: 전날 점수의 일부(10%)를 오늘로 이월한다. 당일 활동이 쌓이면 이월 점수의 영향이 자연스럽게 줄어든다.
...
23:50에 실행해서 자정 전에 내일 키를 미리 준비한다. 자정이 되면 이월된 점수로 시작하고, 실제 활동이 쌓이면서 이월 점수의 비중이 줄어든다.

## 멱등성 보장

스케줄러가 여러 번 실행되더라도 결과가 동일해야 한다. `ZADD`의 `NX` 옵션을 사용하면 이미 존재하는 멤버는 업데이트하지 않는다.
...
단, 이 방식은 "아직 오늘 활동이 없는 신규 상품만 이월 점수로 초기화"하는 의미다. 오늘 이미 활동이 있었다면 이월 점수는 무시된다.
...
TTL을 48시간으로 설정하면 어제 키가 오늘 자정 이후에도 24시간 더 유지된다. 어제 랭킹 조회가 필요할 때 사용할 수 있다.
...
Redis ZSET은 랭킹 구현에 최적화된 자료구조다. `ZINCRBY`의 아토믹 연산 덕분에 동시성 처리가 자연스럽다.

콜드 스타트 문제는 실시간 랭킹 시스템의 숨겨진 함정이다. 단순히 "자정에 초기화"가 아니라, 연속성 있는 랭킹을 위해 이월 전략이 필요하다.

Score Carry-Over 방식은 다음 장점이 있다:
- 자정에 랭킹이 비지 않는다
- 당일 활동이 없는 상품은 자연스럽게 순위가 낮아진다
- 전날 인기 상품이 당일 초기에도 어느 정도 반영된다

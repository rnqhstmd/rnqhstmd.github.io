...
`@MockitoSpyBean`은 실제 Bean을 Spring Context에서 가져오되, 특정 메서드만 선택적으로 stubbing할 수 있습니다.
...
**통합 테스트는 "결과"를 검증합니다.** Mock 기반 테스트는 "행동"을 검증합니다. 솔직히 통합 테스트에서 `verify`만 쓰고 `assertThat`이 없다면, 그건 테스트 전략이 잘못된 겁니다. 처음 이 코드를 보고 당황했습니다.
...
각 레이어가 검증해야 할 것을 명확히 하고, 그에 맞는 도구를 선택해야 합니다. `@MockitoSpyBean`은 통합 테스트 레이어에서 외부 의존성을 격리할 때 딱 맞는 도구입니다.

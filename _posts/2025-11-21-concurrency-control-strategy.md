...
주문 기능을 구현하다 보면 세 가지 도메인에서 동시성 이슈가 발생한다.

- **재고 (Product)**: 동시에 10명이 마지막 1개를 주문하면?
- **포인트 (Point)**: 동시에 여러 결제가 같은 포인트를 차감하면?
- **쿠폰 (Coupon)**: 동시에 여러 사람이 같은 쿠폰을 사용하면?

처음에는 세 도메인 모두 비관적 락(Pessimistic Lock)을 적용했다. 일단 안전하게 가자는 생각이었다.

## 비관적 락과 낙관적 락 복습

**비관적 락 (Pessimistic Lock)**
- "충돌이 발생할 것이다"라고 가정하고 미리 잠근다
- `SELECT ... FOR UPDATE`
- 충돌이 많을 때 유리하다
- 데드락 위험이 있다

**낙관적 락 (Optimistic Lock)**
- "충돌이 없을 것이다"라고 가정하고 커밋 시점에 충돌을 감지한다
...
**재고와 포인트**는 충돌 확률이 높다. 인기 상품은 동시에 수십 명이 주문할 수 있고, 포인트도 한 사용자가 여러 탭에서 동시에 결제 시도할 수 있다. 비관적 락이 맞다.

**쿠폰**은 다르다. 보통 쿠폰은 1인 1매 제한이 있고, 같은 쿠폰으로 동시 요청이 들어오는 경우가 드물다. 낙관적 락으로 충돌을 감지하고 예외를 던지는 게 더 가볍다.
...
낙관적 락은 `@Version` 필드를 통해 동시 수정을 감지한다. 두 트랜잭션이 같은 `version`으로 업데이트하려 하면 `OptimisticLockException`이 발생한다.
...
낙관적 락은 예외가 발생했을 때 처리 전략이 필요하다.

쿠폰의 경우, 이미 사용된 쿠폰이라는 의미이므로 재시도가 아닌 명확한 에러 응답을 주는 게 맞다.
...
재고나 포인트처럼 재시도가 의미 있는 경우에는 `@Retryable`을 활용할 수 있지만, 이 경우는 비관적 락을 쓰는 게 더 직관적이다.

## 정리

모든 락을 동일하게 처리하는 건 잘못된 접근이다. 도메인의 특성을 이해하고 적절한 전략을 선택해야 한다.

- **충돌 확률이 높고, 실패가 치명적**: 비관적 락
- **충돌 확률이 낮고, 실패를 예외로 처리 가능**: 낙관적 락

비관적 락은 안전하지만 성능 비용이 있다. 낙관적 락은 가볍지만 충돌 처리 로직이 필요하다. 도메인을 이해한 설계가 과잉 동기화를 막는다.

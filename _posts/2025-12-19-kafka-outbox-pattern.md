...
지난 주 `ApplicationEvent`로 트랜잭션을 분리했다. 그런데 이 구조의 한계가 명확해졌다.

**1. 메모리 기반 - 서버 재시작 시 유실**

이벤트가 메모리에만 존재한다. 이벤트를 발행하고 핸들러가 처리하기 전에 서버가 재시작되면 이벤트는 사라진다.

**2. 순서 보장 없음**

비동기 핸들러가 여러 개일 때 실행 순서를 보장할 수 없다.

**3. 재시도 없음**

핸들러가 실패하면 이벤트가 손실된다. 재시도 로직을 직접 구현해야 한다.

## Kafka를 선택한 이유

메시지 브로커 선택지는 여러 가지다 (RabbitMQ, ActiveMQ 등). Kafka를 선택한 핵심 이유는 **재생(Replay) 기능**이다.

- **내구성**: 메시지가 디스크에 저장된다. 컨슈머가 다운돼도 재시작 후 이어서 처리할 수 있다.
- **재생**: 과거 메시지를 다시 처리할 수 있다. 버그 수정 후 재처리, 새 서비스 투입 시 히스토리 처리에 유용하다.
- **순서 보장**: 같은 파티션 내에서 순서가 보장된다.

## 문제: 메시지 발행과 DB 저장의 원자성

Kafka 도입 전에 해결해야 할 근본적인 문제가 있다.
...
DB에 주문이 저장됐는데 Kafka 발행이 실패하면 이벤트가 유실된다. 반대로 Kafka 발행은 됐는데 DB 커밋이 실패하면 유령 이벤트가 된다.

DB 트랜잭션과 Kafka 발행은 원자적으로 처리돼야 한다. 그러나 두 시스템은 같은 트랜잭션을 공유할 수 없다.

## 해결: Transactional Outbox Pattern

핵심 아이디어: **이벤트를 Kafka에 직접 발행하지 않고, 같은 DB 트랜잭션 안에 `outbox` 테이블에 저장한다.**
...
주문 생성과 이벤트 저장이 같은 트랜잭션이다. 주문이 롤백되면 이벤트도 롤백된다. 원자성이 보장된다.
...
`.get()`으로 동기 확인을 하는 이유: 발행 성공이 확인된 후에만 `published = true`로 표시해야 한다. 비동기로 처리하면 발행 실패를 감지하기 어렵다.

## Consumer: 멱등성 보장

Outbox 패턴은 At Least Once 전송을 보장한다. 즉, 같은 메시지가 여러 번 전달될 수 있다. 컨슈머는 멱등하게 처리해야 한다.
...
`event_handled` 테이블에 처리한 이벤트 ID를 저장해서 중복 처리를 방지한다.

## Partition Key로 순서 보장

같은 주문의 이벤트는 같은 파티션에서 처리돼야 순서가 보장된다.
...
같은 `orderId`를 가진 메시지는 항상 같은 파티션으로 라우팅된다.

## DLQ (Dead Letter Queue) 처리

처리를 반복해도 실패하는 메시지는 DLQ로 보낸다.
...
DLQ에 쌓인 메시지는 별도로 모니터링하고 처리한다.

## At Least Once + Idempotency = Exactly Once Semantics

Kafka는 기본적으로 At Least Once를 보장한다. 완벽한 Exactly Once를 달성하려면:
...
각 요소가 역할을 분담해서 전체적으로 정확히 한 번 처리를 달성한다.

## 정리

`ApplicationEvent`의 한계를 Kafka + Transactional Outbox Pattern으로 해결했다.
...
복잡도가 높아진 만큼 신뢰성이 올라갔다. 이벤트 유실이 치명적인 비즈니스 로직에는 Outbox Pattern이 적합하다.

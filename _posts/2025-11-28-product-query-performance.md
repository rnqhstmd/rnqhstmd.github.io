...
`EXPLAIN` 결과를 보니 `type: ALL`이었습니다. 풀 테이블 스캔입니다. 10만 건을 모두 읽고 필터링하고 있었습니다.
...
인덱스 컬럼 순서가 중요합니다. `WHERE brand = ?` 조건이 먼저 오고 `ORDER BY price`가 이어지는 패턴이므로 `(brand, price)` 순서가 맞습니다.
